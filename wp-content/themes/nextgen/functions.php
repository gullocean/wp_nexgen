	...
	try {
		if( isset( $_POST['nonce'] ) && wp_verify_nonce( $_POST['nonce'], 'nonce_quote_form' )){
			...
			if( $response === true ){
				...
				nexgen_return_json_msg( $response, 200 );
			} else {
				nexgen_return_json_msg( $response, 500 );
			}
		} else {
...
add_action( 'wp_ajax_nopriv_nexgen_quote_form', 'ajax_quote_form' );
...
function nexgen_data_handling( $form ){
	...
	// This is a way to figure out which form was submitted
	if( $form['source'] == 'front-page-form' ){
        $data['wl_leadtype'] = 'FREEWEBFORM';
	} elseif( $form['source'] == 'full-page-form' ){
        $data['wl_leadtype'] = 'SWITCHCONSULTATION';
	} else {
		$data['wl_leadtype'] = null;
	}
	...
	/*if(isset($_COOKIE['uniqueid'])) { 							//also tried CampaignID
		...
		// It is possible to select one, few or all business_solution or enquiry_reasons
		// In the way this is coded by now, only the latest one (lowest one) will be saved
		// Check my little code update below
...
// maybe try this approach instead
	...
	$data['wl_typeofsol'] = array();
	$data['wl_reasonenq'] = array();
    ...
    $data['wl_typeofsol'] = implode( " ", $data['wl_typeofsol'] );
    $data['wl_reasonenq'] = implode( " ", $data['wl_reasonenq'] );
...
function nexgen_email_confirmation( $data ) {
	...
	$emails = array(
		'gullocean@gmail.com'
		//'william@webmarketers.com.au'
		//'margotdavis@nexgen.com.au',
		//'bradArgaet@nexgen.com.au',
		//'william@webmarketers.com.au',
		//'william@jimmydata.com',
		//'ioanus1010@gmail.com',
		//'ioanus1010@yahoo.com'
	);

	$emails = 'gullocean@gmail.com';
